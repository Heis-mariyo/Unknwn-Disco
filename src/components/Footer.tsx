...
export function Footer() {
  // 2. Updated structure: Objects with 'name' and 'path'
  const navLinks = [
    { name: 'Home', path: '/' },
  ...
  return (
    <footer className="px-[6%] pt-16 pb-8 text-un-text-gray border-t border-un-border mt-10">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
        ...
        {/* Column 1: Logo */}
        <div>
          <div className="text-xl font-bold tracking-wide mb-4 text-un-purple">
            UNKNWN DISCO
          </div>
        ...
        {/* Column 2: Links */}
        <div>
          <h4 className="font-bold mb-4 text-un-purple">NAVIGATE</h4>
          <ul className="space-y-2">
            {navLinks.map((item) => (
              <li key={item.name}>
                {/* 3. Use Link component with correct href */}
                <Link href={item.path} className="hover:text-un-purple transition-colors">
                  {item.name}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <div className="flex gap-4">
            <a href="#" className="w-10 h-10 rounded-full border border-un-border-ghost flex items-center justify-center hover:text-un-purple hover:border-un-purple text-un-purple transition-all">
              <TwitterIcon />
            </a>
            <a href="#" className="w-10 h-10 rounded-full border border-un-border-ghost flex items-center justify-center hover:text-un-purple hover:border-un-purple text-un-purple transition-all">
              <InstagramIcon />
